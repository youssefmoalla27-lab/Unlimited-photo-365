...
## Résolution d'erreurs de build Vercel

### Erreur : `tsc` affiche l'aide au lieu de compiler

**Cause :** `tsconfig.json` absent → TypeScript ne trouve pas de projet et affiche l'aide générale, ce qui fait échouer le build avec exit code 1.

**Correction appliquée :** `backend/tsconfig.json` est maintenant inclus dans le projet.

---

### Déploiement backend sur Vercel

Vercel compile le TypeScript directement via `@vercel/node`, **sans appeler `tsc` manuellement**. Le `vercel.json` inclus dans `backend/` configure cela.
...
> ⚠️ Limite Vercel : timeout 10s, payload max 4.5MB. Pour les uploads jusqu'à 50MB, préférez Railway.

### Déploiement recommandé — Railway

Railway supporte Express persistant, bien mieux pour les uploads :
...
Railway provisionne PostgreSQL en un clic depuis le dashboard.
